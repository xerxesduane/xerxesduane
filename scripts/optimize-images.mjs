...
// work-raw/ is gitignored, so a fresh clone has no sources. Without this guard
// the loop above finds nothing and the manifest below is rewritten as an empty
// array, silently deleting every portfolio item. Bail out instead.
if (items.length === 0) {
  console.error(
    "No source images found under work-raw/. Nothing was written.\n" +
      "Drop the originals into work-raw/web and work-raw/graphic first.",
  );
  process.exit(1);
}
